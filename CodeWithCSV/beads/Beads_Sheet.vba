...
If Not IsNull(Me![Unit]) And Not IsNull(Me![Sample]) And Not IsNull(Me![findsampleletter]) Then
    Me![GID] = Me![Unit] & "." & Me![findsampleletter] & "." & Me![findnumber]
Else
    Debug.Print "GID is not valid"
End If
End Sub

Private Sub analyst_Change()
    [timestamp].Value = Now()
End Sub
...
Dim response
    ...
    If response = vbYes Then
        Dim sql
        sql = "Delete FROM [Beads: Basic Details] WHERE [GID] = '" & Me![GID] & "';"
        DoCmd.RunSQL sql
        
        sql = "Delete from [Beads: Dendro] WHERE [GID] = '" & Me![GID] & "';"
        DoCmd.RunSQL sql
...
Private Sub flotation_number_Change()
    [timestamp].Value = Now()
End Sub
