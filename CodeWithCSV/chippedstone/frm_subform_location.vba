...
On Error GoTo err_locate
    ...
    Dim current
    current = "BAG" & Forms![frm_CS_basicdata]![txtBag] & ":" & Forms![frm_CS_basicdata]![Unit] & ":"
...
On Error GoTo err_move
    ...
    '    Else
    '        'ok only one location so now is it a GS location?
    '        If Me![CrateLetter] <> "GS" Then
    '            MsgBox "This particular find is listed as being located in non Groundstone Crate. This may mean it has been mis-assigned or " & _
    ...
    '                "updating the location for you.", vbExclamation, "Raise with Finds Officer"
    '        Else
    '            'ok so allow it to be moved within the GS crates
    '            'get find number from main form to pass as openargs
    '            current = Forms![Frm_Basic_Data]![frm_subform_basic].Form![Unit] & ":" & Forms![Frm_Basic_Data]![frm_subform_basic].Form![LetterCode] & ":" & Forms![Frm_Basic_Data]![frm_subform_basic].Form![FindNumber] & ":" & Me![CrateLetter] & Me![CrateNumber]
    ...
    'ElseIf Me![txtUnit] <> "" Then
        ...
        Else
            'ok only one location so now is it a GS location?
            If Me![CrateLetter] <> "OB" Then
                MsgBox "This particular bag is listed as being located in non ChippedStone Crate. This may mean it has been mis-assigned and " & _
                    " needs to be checked. Please take this issue to the Finds Officer " & _
                    "straight away who can resolve it and ensure the Crate Register is updated correctly." & Chr(13) & Chr(13) & "The Finds Officer will then deal with " & _
                    "updating the location for you.", vbExclamation, "Raise with Finds Officer"
            Else
                'ok so allow it to be moved within the GS crates
                'get find number from main form to pass as openargs
                'current = "BAG" & Forms![Frm_CS_BasicData]![txtBag] & ":" & Forms![Frm_CS_BasicData]![Unit]
                current = "ROW" & Me![rowID] & ":" & Me![CrateLetter] & Me![CrateNumber]
                DoCmd.OpenForm "frm_subform_changelocation", acNormal, , , acFormPropertySettings, acDialog, current
            End If
        End If
    'End If
...
'check if there is any known crate location for the selected record (this form is only opened via frm_cs_basic_data)
