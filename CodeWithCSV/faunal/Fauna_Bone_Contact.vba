...
On Error GoTo Err_button_goto_measurement_Click
    ...
    '2010 added update gid here - seems some problem with getting this GID info and saving
    'have disabled the save as was causing hash delete and not actually helping
    'to overcome save issue am closing this form at bottom of script
    UpdateGID
...
If Me![GID] <> "" Then
    ...
    ''DoCmd.Minimize 'see below
    DoCmd.OpenForm stDocName, , , stLinkCriteria
    '2010 saving is causing hash deleted - lets see if closing will help - how do they return tothis
    'form? will it cause problems?
...
Err_button_goto_measurement_Click:
    If Err.Number = 3146 Then
        MsgBox "There is a problem with this record. Does this record exist in the BFD? If it does not exist there you will receive a FOREIGN KEY error as it must. Error message to follow.", vbCritical, "Error"
    End If
    Call General_Error_Trap
...
temporder = ContactOrder
    ...
    'RISSA HERE IS THE CHANGE
    'saj july 2007 - increment of contact order not working - surprise, surprise....its not saving the record to dB before getting the number
    'DoCmd.RunCommand acCmdSaveRecord 'for some reason having a save was resulting #Deleted appearing ???
    Me.Requery 'this will do for the moment but the screen will flash.
...
If Me![GID] <> "" Then
    'july 2007 - the save is causing #deleted
    Dim tempGID
    Dim tempUnit
    Dim tempLetter
    Dim tempFind
    ...
    'If [Forms]![Fauna_Bone_Basic_Faunal_Data].[Unit number] = 0 Then
        'stLinkCriteria = "[GID]=" & "'" & Me![GID] & "'"
        stLinkCriteria = "[GID]=" & "'" & tempGID & "'"
    'End If
        
    
    'new for season 2007 - see if the artifact record exists if not create it - SAJ
    'checknum = DLookup("[GID]", "[Fauna_Bone_Artifacts]", "[GID] = '" & Me![GID] & "'")
    checknum = DLookup("[GID]", "[Fauna_Bone_Artifacts]", "[GID] = '" & tempGID & "'")
    If IsNull(checknum) Then
        'gid not exist there yet
        ...
        retVal = MsgBox("No Artifact record exists yet for GID " & tempGID & ", do you want to create one now?", vbQuestion + vbYesNo, "Create New Modification Record")
        If retVal = vbNo Then
            'do nothing
            Exit Sub
        Else
            ...
            sql = "INSERT INTO [Fauna_Bone_Artifacts] ([GID], [Unit number], [Letter code], [Find number]) VALUES ('" & tempGID & "'," & tempUnit & ", '" & tempLetter & "', " & tempFind & ");"
            DoCmd.RunSQL sql
        End If
    End If
    ...
    '[Forms]![Fauna_Bone_Artifacts].[unit number] = [Forms]![Fauna_Bone_Contact].[unit number]
    '[Forms]![Fauna_Bone_Artifacts].[letter code] = [Forms]![Fauna_Bone_Contact].[letter code]
    '[Forms]![Fauna_Bone_Artifacts].[Find number] = [Forms]![Fauna_Bone_Contact].[Find number]
    'if we are going to do this should do GID as well - added by SAJ
    '[Forms]![Fauna_Bone_Artifacts].[GID] = [Forms]![Fauna_Bone_Contact].[GID]
    [Forms]![Fauna_Bone_Artifacts].[Unit number] = tempUnit
    [Forms]![Fauna_Bone_Artifacts].[letter code] = tempLetter
    [Forms]![Fauna_Bone_Artifacts].[find number] = tempFind
    'if we are going to do this should do GID as well - added by SAJ
    [Forms]![Fauna_Bone_Artifacts].[GID] = tempGID
...
'2008 Rissa says focus not always on location field
DoCmd.GoToControl "Field118"
Exit Sub
...
'2008 Rissa reported sometimes losing GID when moving to measurements table so putting extra
'save here to see if helps
