...
On Error GoTo err_cmdNewSubType_Click
    ...
    If Forms![Finds: Admin_MaterialGroupSubGroupLOV]![MaterialID] <> "" Then
        Dim sql, retVal
        retVal = InputBox("Please enter the new subtype for the material type '" & Forms![Finds: Admin_MaterialGroupSubGroupLOV]![MaterialGroup] & "': ", "Enter new subtype")
        If retVal <> "" Then
            
            'new 2008 - check this subgroup text not exist already
            Dim checkit
            checkit = DLookup("[MaterialGroupID]", "[Finds_Code_MaterialGroup_Subgroup]", "[MaterialGroupID] = " & Forms![Finds: Admin_MaterialGroupSubGroupLOV]![MaterialID] & " AND [MaterialSubgroupText] = '" & retVal & "'")
            If IsNull(checkit) Then
                sql = "INSERT INTO [Finds_Code_MaterialGroup_Subgroup] ([MaterialGroupID], [MaterialSubgroupText]) VALUES (" & Forms![Finds: Admin_MaterialGroupSubGroupLOV]![MaterialID] & ", '" & retVal & "');"
                DoCmd.RunSQL sql
                Me.Requery
            Else
                MsgBox "This material subgroup already exists for " & Forms![Finds: Admin_MaterialGroupSubGroupLOV].Form![Exca: Admin_Subform_FeatureSubType]![MaterialSubgroupText] & ". Please use the record selectors at the bottom of the green box to locate it", vbInformation, "Sub group already exists"
            End If
        End If
    Else
...
'2008 - make combo if new record or else lock as had problem with over writing
On Error GoTo err_current

    If Me![MaterialSubgroupText] = "" Or IsNull(Me![MaterialSubgroupText] = "") Then
        Me![cboMaterialSubgroupText].Visible = True
        Me![txtMaterialSubgroupText].Visible = False
    Else
        Me![txtMaterialSubgroupText].Visible = True
        Me![cboMaterialSubgroupText].Visible = False
    End If
Exit Sub
