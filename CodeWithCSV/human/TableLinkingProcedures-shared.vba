...
If username <> "" And pwd <> "" Then
    ...
    For I = 0 To mydb.TableDefs.Count - 1 'loop the tables collection
        ...
        '18th july 2009 - for some reason a tmp table is the first table to be picked up with connect
        'string, maybe some used this version and it crashed? This means at the refresh link line the system
        'goes to error trap that says 'unrecognised database format' and crashes. Temp fix is to intercept here
        'really needs fresh msb shell. SAJ (and part of if statement added below)
