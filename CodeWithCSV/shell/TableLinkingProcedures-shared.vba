...
SetGeneralPermissions username, pwd, connStr 'use this is you want to know what level of priviledge the user has
...
Function ReAttachAllTables()
' Relink all tables at once when linked table manager prompts for the DSN on every occassion
' SAJ-13/09/06 - post 2006 season
    ...
    For I = 0 To mydb.TableDefs.Count - 1 'loop the tables collection
        ...
        If tmptable.Connect <> "" Then
            ...
            '    On Error GoTo err_reattach:
                ...
                'ODBC;DSN=catalhoyuk;APP=Microsoft® Access;WSID=MHJPX2K40B3WMP;DATABASE=catalhoyuk;Network=DBMSSOCN
                'ODBC;DSN=catalcam;Description=Catalhoyuk (Cambridge) Database Server;APP=Microsoft® Access;WSID=MOWMOW1;DATABASE=catalhoyuk;Network=DBMSSOCN;Address=catalsql.arch.cam.ac.uk,1434
                Dim linkto
                'linkto = "ODBC;DSN=catalhoyuk;APP=Microsoft® Access;DATABASE=catalhoyuk;Network=DBMSSOCN"
                linkto = "ODBC;DSN=catalcam;Description=Catalhoyuk (Cambridge) Database Server;APP=Microsoft® Access;DATABASE=catalhoyuk;Network=DBMSSOCN;Address=catalsql.arch.cam.ac.uk,1434"
                tmptable.Connect = linkto
                tmptable.RefreshLink
            'End If
...
err_reattach:
    ...
    '    AlterODBC
    Else
        MsgBox Err.DESCRIPTION & Chr(13) & Chr(13), vbCritical, "Login Failure"
    End If
End Function
